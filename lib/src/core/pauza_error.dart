...
sealed class PauzaError implements Exception {
  ...
  factory PauzaError.fromPlatformException(PlatformException exception) {
    final message =
        exception.message ?? 'A platform error occurred (${exception.code})';
    switch (exception.code) {
      case 'UNSUPPORTED':
        return PauzaUnsupportedError(
          message: message,
          details: exception.details,
          rawCode: exception.code,
        );
      case 'MISSING_PERMISSION':
        return PauzaMissingPermissionError(
          message: message,
          details: exception.details,
          rawCode: exception.code,
        );
      case 'PERMISSION_DENIED':
        return PauzaPermissionDeniedError(
          message: message,
          details: exception.details,
          rawCode: exception.code,
        );
      case 'SYSTEM_RESTRICTED':
        return PauzaSystemRestrictedError(
          message: message,
          details: exception.details,
          rawCode: exception.code,
        );
      case 'INVALID_ARGUMENT':
        return PauzaInvalidArgumentError(
          message: message,
          details: exception.details,
          rawCode: exception.code,
        );
...
final class PauzaMissingPermissionError extends PauzaError {
  const PauzaMissingPermissionError({
    required super.message,
    super.details,
    super.rawCode,
  });
...
final class PauzaPermissionDeniedError extends PauzaError {
  const PauzaPermissionDeniedError({
    required super.message,
    super.details,
    super.rawCode,
  });
...
final class PauzaSystemRestrictedError extends PauzaError {
  const PauzaSystemRestrictedError({
    required super.message,
    super.details,
    super.rawCode,
  });
...
final class PauzaInvalidArgumentError extends PauzaError {
  const PauzaInvalidArgumentError({
    required super.message,
    super.details,
    super.rawCode,
  });
...
final class PauzaInternalFailureError extends PauzaError {
  const PauzaInternalFailureError({
    required super.message,
    super.details,
    super.rawCode,
  });
  ...
  Future<T> throwTypedPauzaError() async {
    try {
      ...
      throw PauzaUnsupportedError(
        message: error.message ?? 'Operation is not supported',
      );
    } catch (error) {
