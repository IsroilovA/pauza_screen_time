...
///
/// This class provides APIs to check, request, and manage permissions
/// separately for Android and iOS platforms.

/// Manages platform-specific permissions.
class PermissionManager {
  final PermissionPlatform _platform;

  PermissionManager(this._platform);
  ...
  ///   final status = await manager.checkAndroidPermission(
  ...
  /// ```dart
  /// if (Platform.isIOS) {
  ///   final status = await manager.checkIOSPermission(
  ...
  /// Example:
  /// ```dart
  /// final statuses = await manager.checkAndroidPermissions([
  ///   AndroidPermission.usageStats,
  ///   AndroidPermission.queryAllPackages,
  /// ]);
  /// if (statuses[AndroidPermission.usageStats]?.isGranted ?? false) {
  ///   // Usage stats is available
  /// }
  /// ```
  ...
  /// Example:
  /// ```dart
  /// final statuses = await manager.checkIOSPermissions([
  ///   IOSPermission.familyControls,
  ///   IOSPermission.screenTime,
  /// ]);
  /// ```
  ...
  /// final allMissing = await manager.getMissingAndroidPermissions();
  /// ```
  ...
  /// Example:
  /// ```dart
  /// final missing = await manager.getMissingIOSPermissions([
  ///   IOSPermission.familyControls,
  /// ]);
  /// ```
